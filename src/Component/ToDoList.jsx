...
function Todolist() {
    let [todos, setTodos] = useState([{ task: "Dummy Task", id: uuidv4(), isDone: false }]);
    ...
    return (
        <>
            ...
            <div className="flex flex-wrap justify-center mt-12 px-4 md:px-8 lg:px-16">
                <input
                    className="w-full md:w-[50%] lg:w-[40%] mx-2 md:mx-4 lg:mx-8 placeholder:italic placeholder:text-slate-400 block bg-white border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1"
                    type="text"
                    placeholder="Add A Task"
                    value={newtodos}
                    onChange={handleTodovalue}
                />
                <button className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-1 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-4 py-2 md:px-5 md:py-2 text-center" onClick={handleNewTask}><i className="fa-solid fa-plus"></i></button>
            </div>
                    ...
                    {todos.map((todo) => (
                        <li className="flex flex-wrap justify-between items-center m-4" key={todo.id}>
                            ...
                            <div className="flex space-x-2 mt-2 md:mt-0">
                                {editingId === todo.id ? (
                                    <button onClick={() => handleEditSubmit(todo.id)} className="text-white bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:focus:ring-yellow-800 font-medium rounded-lg text-sm px-4 py-2 text-center"><i className="fa-solid fa-check"></i></button>
                                ) : (
                                    <button onClick={() => handleEdit(todo.id, todo.task)} className="text-white bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:focus:ring-yellow-800 font-medium rounded-lg text-sm px-4 py-2 text-center"><i className="fa-solid fa-pen"></i></button>
                                )}
                                ...
                                <button onClick={() => handleDonetask(todo.id)} className="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-4 py-2 text-center"><i className="fa-solid fa-check"></i></button>
                            </div>
                        </li>
                    ))}
                </ul>
            ...
            <div className="flex justify-center m-3">
                <button onClick={handleDoneAllTasks} className="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2 text-center">Done All Tasks</button>
            </div>
